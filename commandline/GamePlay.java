...
//			Initialize deck
			...
			if (logCheck == true) {
				TopTrumpsCLIApplication.testlog.log(Level.FINE,
						"The contents of the complete deck once it has been read in and constructed: ");
				deckList = mainDeck.getDeck();
				deckList.forEach(Card -> TopTrumpsCLIApplication.testlog.log(Level.FINE, Card.toString()));
			}
...
//			Deal cards to players - Needs work! - What is if this division does not work?
			...
			if (logCheck == true) {
				...
				for (Player player : players) {
					TopTrumpsCLIApplication.testlog.log(Level.FINE, player.getPlayerID());
					player.getHand().forEach(Card -> TopTrumpsCLIApplication.testlog.log(Level.FINE, Card.toString()));
				}
		...
		if (activePlayer.equals(user)) {
			...
			do {
				...
				if (TopTrumpsCLIApplication.writingTestLogs == true) {
					...
					TopTrumpsCLIApplication.testlog.log(Level.FINE, "Category Selected: " + chosenCategory);
					for (Player player : players) {
						TopTrumpsCLIApplication.testlog.log(Level.FINE,
...
//		//In CardDecks in the Add/Remove CommonPileSection at the end
		if (logCheck == true) {
			...
			commonPile.forEach(Card -> TopTrumpsCLIApplication.testlog.log(Level.FINE, Card.toString()));
		}
	}
...
//		//In Gameplay at the position when cards are compared - getCardsInPlay method needed
		if (TopTrumpsCLIApplication.writingTestLogs == true) {
			...
			for (Player player : players) {
				TopTrumpsCLIApplication.testlog.log(Level.FINE, player.getHand().toString());
			}
		}
		for (Player player : players) {
			if (!player.equals(roundVictor)) {
